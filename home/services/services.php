...
if (isset($_POST["submit"])) {
    ...
    $query = mysqli_query($konek, "UPDATE urls SET dash_name='$dash_name', url1='$url1', url2='$url2' WHERE id=$id");
...
if (isset($_POST['create-dashboard'])) {
    ...
    if (!isValidUrl($url1)) {
        echo "<script>alert('URL 1 tidak valid atau tidak dapat diakses.');</script>";
        // } elseif (!empty($url2)) { 
        //     echo "<script>alert('URL 2 tidak valid atau tidak dapat diakses.');</script>";
    } else {

        $sql = "INSERT INTO urls (id, dash_name, url1, url2) VALUES 
            (NULL, '$dash', '$url1', '$url2')";
        $konek->query($sql);
...
$grafana = "SELECT id, dash_name, url1, url2 FROM urls";
                        ...
                        <div class="card" style="background: #05a2e4; box-shadow: 0 4px 8px 0 rgba(0, 0, 0, 0.2), 0 6px 20px 0 rgba(0, 0, 0, 0.19);">
                            ...
                            <div class="collapse" id="collapse<?= $row['id'] ?>">
                                <div class="card-body" style="background: #E5E7EB;">
                                    <div class="row">
                                        <div>
                                            <iframe src="<?= $row['url1'] ?>&kiosk" width="100%" height="<?= $row['url2'] ?>" frameborder="0"></iframe>
                                        </div>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="modal-content">
                                ...
                                <div class="modal-body">
                                    <form action="?id=<?= $row['id'] ?>" method="post">
                                        ...
                                        <div class="mb-3">
                                            <label for="url2" class="form-label">Height</label>
